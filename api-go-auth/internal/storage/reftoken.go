...
func NewRefreshTokenStorage(q pgxtype.Querier) models.RefreshTokenStorage {
	return &RefreshTokenStorage{conn: q}
}
...
func (r *RefreshTokenStorage) RevokeAllByUserId(userId int64) error {
	...
	_, err := r.conn.Exec(context.Background(), `UPDATE refresh_tokens SET "isRevoked" = TRUE WHERE "userId" = $1 AND "isRevoked" IS FALSE`, userId)
	return err
}
